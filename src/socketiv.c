...
static inline int socketiv_alter_fd(int fd, SOCKETIV_FD_TYPE fd_type){ // modify a fd_type entry of global list 
...
void socketiv_register_fd(int fd, SOCKETIV_FD_TYPE fd_type){ // register a fd_type entry to global list
	if(fd >= fd_list_reserve){
		fd_list = realloc(fd_list, sizeof(SOCKETIV_FD_TYPE) * (fd_list_reserve * 2));
		memset(fd_list + fd_list_reserve, SOCKETIV_FD_TYPE_INV, fd_list_reserve);
		fd_list_reserve *= 2;
	}
	if(fd >= fd_list_size){
...
SOCKETIV_FD_TYPE socketiv_check_fd(int fd){ // check type of a fd_type entry in global list
	return fd_list[fd];
}
...
bool socketiv_check_vm_subnet(const struct sockaddr *addr){ // determine whether this address belongs to virtual network
...
ssize_t socketiv_write(int fd, const void *buf, size_t count){
	...
	// - 블록 사이즈 보다 작은 write의 경우: 지정된 delay time window 만큼 대기후에 인터럽트(이때는 메인 함수내에서 block 하지 않음)
